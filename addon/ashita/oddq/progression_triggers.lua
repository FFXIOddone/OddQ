...
local function snapshot(live)
    ...
    return {
        ...
        in_cutscene = live.in_cutscene == true,
        cutscene_event_id = tonumber(live.cutscene_event_id),
        cutscene_zone_id = tonumber(live.cutscene_zone_id or live.current_zone_id),
    }
end
...
function progression_triggers.observe_live(state, live)
    ...
    if previous.in_cutscene ~= true and current.in_cutscene == true then
        table.insert(events, progress_event("cutscene_started", {
            event_id = current.cutscene_event_id,
            zone_id = current.cutscene_zone_id,
        }))
    elseif previous.in_cutscene == true and current.in_cutscene ~= true then
        table.insert(events, progress_event("cutscene_finished", {
            event_id = previous.cutscene_event_id,
            zone_id = previous.cutscene_zone_id,
        }))
    end
...
local function event_matches(condition, event)
    if type(condition) ~= "table" or type(event) ~= "table"
        or tostring(condition.event or "") ~= tostring(event.event or "") then
        return false
    end
    local expected_name = condition.name
    local actual_name = event.item_name or event.key_item_name
    if expected_name ~= nil and normalized(expected_name) ~= normalized(actual_name) then return false end
    if condition.event_id ~= nil and tonumber(condition.event_id) ~= tonumber(event.event_id) then return false end
    if condition.zone_id ~= nil and tonumber(condition.zone_id) ~= tonumber(event.zone_id) then return false end
    return true
end

function progression_triggers.step_completed(step, events)
    local conditions = type(step) == "table" and step.auto_advance_events or nil
    if type(conditions) ~= "table" or #conditions == 0 then return false end
    for _, condition in ipairs(conditions) do
        for _, event in ipairs(events or {}) do
            if event_matches(condition, event) then return true, event end
        end
    end
    return false
end
